...
module.exports = async (req, res) => {
  ...
  try {
    if (req.method === 'PUT') {
      ...
      if (!updates.length) {
        const rawPhone = auth.user.phone ? safeDecrypt(auth.user.phone) : ''
        return res.json(success({
          ...
          publish_permission: parseInt(auth.user.publish_permission || 0, 10) || 0,
          ...(await getRiskSnapshot(auth.user.id))
        }))
      }
      ...
      const risk = await getRiskSnapshot(auth.user.id)

      const decryptedPhone = freshUser?.phone ? safeDecrypt(freshUser.phone) : ''
      ...
      return res.json(success({
        ...
        total_earnings: parseFloat(freshUser.total_earnings || 0),
        available_balance: parseFloat(freshUser.available_balance || 0),
        frozen_balance: parseFloat(freshUser.frozen_balance || 0),
        publish_permission: parseInt(freshUser.publish_permission || 0, 10) || 0,
        ...risk
      }))
    }
