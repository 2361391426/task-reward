...
const initDB = () => {
  ...
  db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS tasks (
      ...
      status INTEGER DEFAULT 1,
      accept_start_time DATETIME DEFAULT NULL,
      end_time DATETIME NOT NULL,
      created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
      updated_at DATETIME DEFAULT CURRENT_TIMESTAMP
    );

    CREATE INDEX IF NOT EXISTS idx_tasks_merchant_status_created_at
      ON tasks (merchant_id, status, created_at);
    CREATE INDEX IF NOT EXISTS idx_tasks_accept_start_time
      ON tasks (accept_start_time);
    CREATE INDEX IF NOT EXISTS idx_tasks_status_end_time
      ON tasks (status, end_time);
      ...
      screenshot_paid_order TEXT NOT NULL,
      address_text TEXT,
    ...
    CREATE INDEX IF NOT EXISTS idx_submissions_task_status_created_at
      ON submissions (task_id, review_status, submit_time);
    CREATE INDEX IF NOT EXISTS idx_submissions_user_status_created_at
      ON submissions (user_id, review_status, submit_time);
