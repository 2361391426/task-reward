...
module.exports = async (req, res) => {
  ...
  try {
    ...
    busboy.on('finish', async () => {
      try {
        if (!fileReceived || !fileBuffer) {
          return res.status(400).json(error(1001, 'No file found'));
        }

        if (fileTooLarge || fileBuffer.length > MAX_FILE_SIZE) {
          return res.status(400).json(error(1001, 'File size cannot exceed 5MB'));
        }

        if (!filename || !/\.(png|jpe?g|webp|gif)$/i.test(filename)) {
          return res.status(400).json(error(1001, 'Only image files are allowed'));
        }

        const detectedType = detectImageType(fileBuffer);
        if (!detectedType || (mimetype && mimetype !== detectedType)) {
          return res.status(400).json(error(1001, 'Invalid image file'));
        }
    ...
    busboy.on('error', (err) => {
      ...
      return res.status(500).json(error(500, 'File parse failed'));
    });
