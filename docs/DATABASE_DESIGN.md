...
CREATE TABLE tasks (
  ...
  requirements TEXT COMMENT '任务要求（JSON格式）',
...
### 7. 商家充值记录表 (merchant_recharges)

```sql
CREATE TABLE merchant_recharges (
  id BIGINT PRIMARY KEY AUTO_INCREMENT,
  merchant_id BIGINT NOT NULL COMMENT '商家ID',
...
1. **用户表**: openid 和 phone 建立索引，用于快速登录和查询
2. **任务表**: merchant_id、status、start_time 建立索引，支持商家任务列表和用户任务筛选
3. **提交记录表**:
   - task_id 和 user_id 建立索引，支持任务提交查询
   - 唯一索引 (task_id, user_id) 防止重复提交
   - status 索引支持审核列表查询
4. **收益记录表**: user_id、type、created_at 建立索引，支持用户收益明细查询

## 数据安全

1. **敏感信息加密**:
   - 用户手机号使用 AES-256 加密存储
   - 商家密码使用 bcrypt 加密
   - 提现账户信息加密存储
